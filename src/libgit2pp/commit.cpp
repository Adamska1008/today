...
namespace libgit2pp
{
    ...
    signature commit::committer() const noexcept
    {
        return git_commit_committer(_commit);
    }

    signature commit::author() const noexcept
    {
        return git_commit_author(_commit);
    }

    std::chrono::time_point<std::chrono::system_clock> commit::time() const noexcept
    {
        return std::chrono::system_clock::from_time_t(git_commit_time(_commit));
    }
 
    std::size_t commit::parentcount() const noexcept
    {
